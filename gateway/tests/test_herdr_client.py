...
@pytest.mark.asyncio
async def test_list_agents_excludes_primary_workspace_but_keeps_real_workers():
    snapshot = json.loads((FIXTURES / 'deployed-captain-routing.json').read_text())
    client = HerdrClient()
    client.get_snapshot = AsyncMock(return_value=snapshot)

    agents = await client.list_fleet_agents()

    assert [agent['id'] for agent in agents] == ['w22:p2']
    assert agents[0]['workspace_role'] == 'worker'
    assert all(agent['workspace_id'] != 'w4' for agent in agents)
    # Filtering and routing consume the same structural role source.
    assert await client.resolve_target('captain') == 'w4:p1'
...
async def test_list_agents_prefers_real_name_over_generic_harness_title_and_never_exposes_ids():
    ...
    assert agents[1]['workspace_role'] == 'primary'
