...
export async function loadChatPreferences(): Promise<ChatPreferences> {
  const [toolCalls, themeMode, background] = await AsyncStorage.multiGet([
...
export function applyChatAppearance(preferences: Pick<ChatPreferences, 'themeMode' | 'background'>): void {
  setThemeModeOverride(preferences.themeMode);
  setActiveBackground(preferences.background);
}
