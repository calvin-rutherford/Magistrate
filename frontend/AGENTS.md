...
The captain thread renders the gateway's canonical conversation record: `fetchCanonicalConversation` plus `conversation_messages` events, merged by `src/services/CanonicalConversation.ts` (append by canonical id, update on revision, order by `sequence_index`). Startup may display the strictly validated `ConversationSession` cache while the full list is in flight or transiently unavailable, but the next successful full list remains authoritative and prunes it. Read `../CHAT_ARCHITECTURE_FIX.md` before changing this, and do not reintroduce text matching, optimistic counting, or prompt-boundary inference.
