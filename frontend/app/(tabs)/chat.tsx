...
export function ChatCanvas({ target = 'captain', showToolCalls = false, onDrawerToggle = () => {}, drawerOpen = false, profiles = [], capabilityLoading = false, capabilityError = null, selectedProfileId = null, routingReady = true, onProfileChange = () => {}, voiceInputMode = 'automatic', voiceCapabilities, autoStartRecording = false, onRegenerate }: { target?: string; showToolCalls?: boolean; onDrawerToggle?: () => void; drawerOpen?: boolean; profiles?: ExecutionProfile[]; capabilityLoading?: boolean; capabilityError?: string | null; selectedProfileId?: string | null; routingReady?: boolean; onProfileChange?: (profileId: string | null) => void; voiceInputMode?: VoiceInputMode; voiceCapabilities?: VoiceInputCapabilities; autoStartRecording?: boolean; onRegenerate?: (message: ConversationMessage) => Promise<void> }) {
  ...
  const [micStatus, setMicStatus] = useState<'idle' | 'listening' | 'transcribing' | 'ready' | 'error'>('idle');
  ...
  const handleMicPress = async () => {
    if (isRecording) {
      ...
      try {
        ...
        setPromptText(previous => previous.trim() ? `${previous.trim()} ${transcript}` : transcript);
        setMicStatus('ready');
        setSendError(null);
      } catch (error) {
        setMicStatus('error');
    ...
    try { await capture.start(); setIsRecording(true); setMicStatus('listening'); }
  ...
  const loadOlderHistory = async () => {
    ...
    try {
      ...
      // Server-known history rows must keep the gateway's stable id. Minting a
      // local id here would break cursor/dedup identity and present a row we
      // cannot re-address, so a row without one is dropped instead.
      prependConversationMessages(target, result.messages.filter(message => typeof message.id === 'string' && message.id.length > 0).map(message => ({ id: message.id as string, role: message.role, kind: message.kind, text: message.text, source: 'text' as const, sources: message.sources, thinkingSummary: message.thinkingSummary, runId: message.runId, regenerateSafe: message.regenerateSafe, progress: message.progress || (message.role === 'assistant' ? 'complete' : undefined) })));
      result.messages.forEach(message => {
        const key = historyKey(message);
        const optimisticCount = optimisticCountsRef.current.get(key) || 0;
        if (message.id && optimisticCount > 0) optimisticCountsRef.current.set(key, optimisticCount - 1);
