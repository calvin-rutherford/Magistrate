...
| Replay and WebSocket catch-up | Conversation/activity replay endpoints, append-only activity change cursor, opt-in `activity_after` delivery, and non-visual frontend catch-up adapter | Gateway `test_canonical_activity.py`, `test_structured_responses.py`, and frontend `canonical-activity.test.ts` |
