...
> Status: completed on 2026-08-26. The evidence inventory, baseline checks, and UX, accessibility, reliability, performance, security/privacy, mobile/web consistency, test coverage, and operations dimensions are covered below. Validation opportunities remain explicitly distinguished from confirmed defects.
...
#### PERF-01 — Terminal polling can overlap and repeatedly renders an unvirtualized transcript (medium; defer implementation to PR #4)

- **Evidence:** `frontend/app/(tabs)/chat.tsx`, `ChatScreen`, calls asynchronous `loadOutput()` on a fixed two-second `setInterval` without an in-flight guard, then stores the complete returned string in one `Text` inside a `ScrollView`. `gateway/app/main.py`, `get_captain_output`, delegates every request to `HerdrClient.read_agent_output`; that method first resolves the captain through a fleet snapshot and then starts a `herdr agent read` subprocess. A slow request can therefore overlap the next tick, and each response replaces and lays out the whole requested transcript rather than a bounded virtualized row set.
- **Impact:** Poll overlap can multiply gateway and subprocess work during latency, while transcript growth increases client layout and reconciliation cost. The exact CPU, memory, and frame-time impact was not benchmarked, so no numeric degradation is claimed.
- **Boundary:** The source-level overlap and unvirtualized rendering are confirmed on the checked-out baseline. PR #4 explicitly adds overlap-poll prevention, bounded history, and virtualized rendering in these terminal surfaces; do not create a duplicate implementation package. After PR #4 merges, revalidate single-flight polling under delayed responses, transcript bounds, viewport preservation, and phone/web rendering with its tests plus a measured long-session scenario.

#### MWC-01 — Voice Mode does not respond to web viewport changes (medium; coordinate with PR #3)

- **Evidence:** `frontend/app/voice.tsx` reads `Dimensions.get('window').width` once at module evaluation, derives the module-scoped `CANVAS_SIZE`, and uses that constant for `Svg` dimensions and `styles.canvasSurface`. `VoiceScreen` does not use `useWindowDimensions` or subscribe to dimension changes. Consequently, resizing the browser after the module loads cannot recompute the voice visualization's geometry or surface size. Native is configured as portrait-only in `frontend/app.json`, while the web target is a static responsive page whose viewport can change without remounting the module.
- **Impact:** The same Voice Mode surface adapts only to its initial width on web, so responsive browser layouts and device emulation can retain stale geometry until a reload. This is a confirmed source-level consistency defect; no claim is made about a specific overflow threshold because runtime browser measurements were not performed.
- **Boundary:** PR #3 changes `voice.tsx` for speech-reactive visuals and reduced motion, but the inspected PR version retains the same module-scoped `Dimensions` calculation. Avoid a competing edit while that PR is active; after it merges, make sizing reactive and verify the Voice surface at phone, tablet, and desktop widths, including a live browser resize without reload.

#### SEC-01 — A shipped shared token does not provide an effective authorization boundary (high)

**Evidence (historical, remediated).** The audited `frontend/src/api/client.ts` embedded the gateway address and a shared device token in client code and sends that value to account, fleet, agent-control, terminal, notification, voice, and provider endpoints. `frontend/src/realtime/socket.ts` also embeds the same token in a WebSocket URL. `gateway/app/auth.py` accepts that exact value by default when `the legacy token setting` is unset and accepts credentials in either `the legacy token header` or the `token` query parameter. A web bundle or installed client necessarily exposes a client-side constant, so anyone who can obtain the client can reproduce this credential. The gateway's broad CORS policy in `gateway/app/main.py` (`allow_origins`, methods, and headers all wildcarded) does not restore an identity or per-device authorization boundary.
...
#### PERF-02 — Native notification monitoring rebuilds multi-provider attention every 10 seconds (medium risk; measure and coordinate with PR #5)

`frontend/app/_layout.tsx`, `RootLayout`, starts the singleton `NotificationManagerService` for the lifetime of the native app. `frontend/src/services/NotificationManager.ts` polls `/notifications/events` immediately and every ten seconds even when `AppState` is `active`; its overlap guard prevents concurrent polls but does not pause or back off by foreground/background state. `gateway/app/main.py`, `get_notification_events`, calls `attention_service.get_unified_attention_items()` before reconciliation. `gateway/app/attention_service.py` performs Herdr and Firstmate snapshot work and then GitHub, Jira, and Teams provider reads sequentially for each request; the Firstmate snapshot and baseline GitHub adapter can start local subprocesses. This confirms recurring cross-provider work, but battery, network, subprocess, provider-rate-limit, and latency effects were not measured. Instrument request duration and provider call counts under foreground, background, offline, and failure states before selecting push, cache, event-driven, or adaptive polling changes. PR #5 changes GitHub/attention caching and data flow, so defer implementation and remeasure after it merges; acceptance should prove bounded provider calls, backoff/jitter, lifecycle cleanup, and no delayed actionable notification.

#### MWC-02 — Web has no equivalent for merged native attention notification delivery (medium opportunity; confirm product parity requirement)

`frontend/app/_layout.tsx`, `RootLayout`, invokes `NotificationManagerService.startMonitoring()` on every platform, but `frontend/src/services/NotificationManager.ts`, `startMonitoring`, immediately returns when `Platform.OS === 'web'`. Native therefore polls `/notifications/events`, requests notification permission, schedules quiet local alerts, handles notification responses, and acknowledges delivered events; web performs none of those actions. The in-app Attention route remains available on both platforms, so this is not evidence that web users cannot inspect attention items manually. It is a confirmed implementation difference but only a validation opportunity until product requirements establish whether background/browser notification parity is intended. Decide and document the supported web behavior; if parity is required, add a web-appropriate permission/delivery and deep-link path without importing native-only notification behavior blindly, and test disabled, denied, foreground, background, click-through, duplicate, and acknowledgement states on both platform families. Merged notifications are baseline functionality rather than active work, while PR #5 changes Attention/Home data flow; coordinate any web delivery work with PR #5 and revalidate after merge.

#### TST-01 — High-risk security boundaries have no automated regression coverage (high coverage risk)

The gateway suite has 23 collected tests, but its only negative authorization assertion is `gateway/tests/test_gateway.py`, `test_health_unauthorized`, for `/api/v1/health`. `gateway/tests/test_account_oauth_github_voice.py`, `test_oauth_providers`, verifies that the connect route redirects but never invokes or rejects the OAuth callback; no test exercises forged, replayed, expired, or attacker-selected state/redirect values from SEC-02. No gateway test covers production token configuration and endpoint-family authorization from SEC-01 or encryption-key absence, tampered ciphertext, migration, and rotation from SEC-04. The backend suite contains three domain-service tests in `backend/tests/test_services.py`; none instantiates `MagistrateConsumer` or tests WebSocket authentication, authorization, payload bounds, origins, or group isolation from SEC-03. These absences are confirmed coverage gaps, not proof of additional runtime defects. Each SEC-01 through SEC-04 remediation package must add the negative and success-path tests named in that finding's required outcome, and those tests must run in the normal documented suite rather than rely on the live script.

#### TST-02 — Frontend behavior has only a terminal-specific web test harness (medium coverage risk; coordinate with PRs #3–#5)

`frontend/package.json` exposes only `test:chat`, which runs Node's test runner against `frontend/tests/chat-terminal.web.test.js`. Its two browser tests cover terminal wheel scrolling and phone-sized composer input. There are no baseline automated component, API-client, route-state, accessibility, native-device, or notification-delivery tests under `frontend/tests/`; in particular, REL-01 error handling, A11Y-01 control semantics, Voice Mode lifecycle/responsiveness, and merged notification permission/deep-link/acknowledgement behavior are unprotected. This is a confirmed inventory gap, not evidence that those behaviors all fail. Establish a maintainable frontend unit/component harness plus representative web and native integration coverage, with deterministic mocks for network, platform, permissions, app state, timers, and audio. Do not duplicate tests already being introduced by PR #3 for ambient/Voice behavior, PR #4 for terminal behavior, or PR #5 for GitHub/Home behavior: merge or rebase those tests first, then fill only the remaining matrix and run the combined suite.

#### TST-03 — Backend and live end-to-end checks are not reproducible from a ready baseline command (medium coverage/operations risk)

`backend/pytest.ini` configures Django test discovery, but the documented baseline command `cd backend && python3 -m pytest -q` cannot start because pytest is unavailable in the checked environment, so the three service tests have unknown current status. `tests/e2e_live_test.py`, `run_e2e_test`, is a standalone live protocol that requires a running broker, WebSocket endpoint, tmux, and agent execution; it is not collected by either component's normal test command, creates a real fleet through the unauthenticated command socket, and has no fixture-owned environment setup or teardown. These are confirmed reproducibility limitations, not failed application behavior. Define a pinned backend test environment and one documented hermetic command for unit/integration checks; retain the live protocol only as an explicitly gated smoke test with prerequisites, isolation, bounded cleanup, and a non-production target. Record CI execution once CI exists; the open PRs currently report no configured checks, so local PR descriptions are not substitutes for this baseline.
